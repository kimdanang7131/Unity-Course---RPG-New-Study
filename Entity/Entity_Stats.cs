...
public class Entity_Stats : MonoBehaviour
{
    ...
    public float GetElementalDamage(out ElementType element, float scaleFactor = 1)
    {
        ...
        float finalDamage = highestDamage + bonuseElementalDamage + weakerElementsDamage;
    ...
    public float GetElementalResistance(ElementType element)
    {
        ...
        float bonuseResistance = major.intelligence.GetValue() * .5f; // 원소저항 -> 지능당 0.5퍼
        ...
        float resistance = baseResistance + bonuseResistance;
        float resistanceCap = 75f;
        float finalResistance = Mathf.Clamp(resistance, 0, resistanceCap) / 100; // 백분율로
    ...
    public float GetArmorMitigation(float armorReduction)
    {
        ...
        float mitigationCap = .85f; // 최대 한계값
    ...
    public Stat GetStatByType(StatType type)
    {
        switch (type)
        {
            ...
            default:
                Debug.LogError("StatType " + type + " not found!");
                return null;
        }
    }
    ...
    public void ApplyDefaultStatSetup()
    {
        ...
        {
            Debug.Log("No default stat setup assinged");
            return;
        }
